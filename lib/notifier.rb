...
class Notifier
  def self.cr_notify(whodunnit, change_request, message)
    ...
    # get the cr approver
    notification_receiver += change_request.approvals.collect(&:user)

    # get the cr collaborators
    notification_receiver += change_request.collaborators

    # add the requestor
    notification_receiver << change_request.user

    # add release_manager
    notification_receiver += User.where(role: 'release_manager')

    # remove duplicate
    notification_receiver = notification_receiver.uniq

    # substract the whodunnit from user list
    notification_receiver -= [whodunnit]
  ...
  def self.ar_notify(whodunnit, access_request, message)
    notification_receiver = []
    notification_receiver += access_request.approvals.collect(&:user)
    notification_receiver += access_request.collaborators
  ...
  def self.ir_notify(_whodunnit, incident_report, message)
    notification_receiver = []

    # add all user
    notification_receiver += User.active
